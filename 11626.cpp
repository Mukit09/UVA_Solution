...
//	freopen("output.txt", "w", stdout);
    ...
    scanf("%d",&t);
   	while(t--)
   	{
		...
		rep(i,0,n)
		{
		    scanf("%lld%lld%s",&stru[i].x,&stru[i].y,ch);
		    if(ch[0]=='N')
		    stru[i].fg=0;
		    else
		    stru[i].fg=1;
        ...
        j=n-1;
        while(CP(MV(pvt,stru[j]),MV(pvt,stru[j-1]))==0)
        j--;
		...
		rep(i,2,n)
		{
		    while(CP(MV(g[j-2],g[j-1]),MV(g[j-2],store[i]))<0)
		    j--;
		    g[j++]=store[i];
		}

        i64 mnx=g[0].x,mny=g[0].y;
        i64 cnt=0;
        k=0;
        rep(i,0,j)
        {
            if(g[i].x<mnx)
            mnx=g[i].x,mny=g[i].y,k=i;
            else if(g[i].x==mnx&&g[i].y<mny)
            k=i,mny=g[i].y;
            if(g[i].fg)
            cnt++;
        }
